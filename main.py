...
def db_get_managers():
    conn = sqlite3.connect('bot_data.db')
    c = conn.cursor()
    rows = c.execute('SELECT user_id, added_at FROM managers ORDER BY id').fetchall()
    conn.close()
    return rows
...
def send_manager_panel(chat_id, extra_text=""):
    """Отправляет панель менеджера — только добавление ключей."""
            ...
            if durations:
                for dur_id, label, price in durations:
                    ...
                    kb.add(types.InlineKeyboardButton(
                        f"🔑 {p_info['name']} — {label} | {avail}/{total} ключей  ➕",
                        callback_data=f"mgr_addkeyDur_{dur_id}"
                    ))
    caption = "🗂 *ПАНЕЛЬ МЕНЕДЖЕРА*\n━━━━━━━━━━━━━━━━━━\n📦 Выбери товар для добавления ключей:"
    if extra_text:
        caption += f"\n\n{extra_text}"
    try:
        bot.send_photo(chat_id, PHOTO_ADMIN, caption=caption, reply_markup=kb, parse_mode="Markdown")
    except Exception:
...
def admin_menu():
    ...
    kb.row(types.InlineKeyboardButton("👔 Менеджеры", callback_data="adm_managers"))
    return kb
...
def handle_all_callbacks(call):
    ...
    if user_id == ADMIN_ID and (data.startswith("adm_") or data.startswith("ff_currency") or data.startswith("adm_editfield_")):
        ...
        if data == "adm_stats":
        ...
        elif data == "adm_noop" or data == "noop":
            bot.answer_callback_query(call.id)
            ...
            kb.add(
                ...
                types.InlineKeyboardButton(f"🔑 Ключи: {keys_avail}/{keys_total} | ➕ Добавить", callback_data=f"adm_addkeyDur_{dur_id}"),
    ...
    # --------- МЕНЕДЖЕР ONLY ---------
    if is_manager(user_id) and (data.startswith("mgr_addkey_") or data.startswith("mgr_addkeyDur_")):
    ...
    if data == "to_main":
        ...
        if not products:
            ...
            for p_id, p_info in products.items():
                ...
                if durations:
                    ...
                    kb.add(types.InlineKeyboardButton(btn_text, callback_data="noop"))
            kb.add(types.InlineKeyboardButton(t['btn_back'], callback_data="to_main"))
        ...
        if key_value:
            bot.edit_message_caption(
                caption=t['buy_success'].format(name=product['name'], price=price, bal=balance-price, key=key_value, keys_left=keys_left),
                chat_id=chat_id, message_id=msg_id, reply_markup=kb, parse_mode="Markdown"
            )
            try:
                try:
                    buyer = bot.get_chat(user_id)
                    uname_notify = f"@{buyer.username}" if buyer.username else f"#{user_id}"
                except Exception:
                    uname_notify = f"#{user_id}"
                bot.send_message(ADMIN_ID,
                    f"🛒 *НОВАЯ ПОКУПКА!*\n\n"
                    f"👤 {uname_notify} (`{user_id}`)\n"
                    f"📦 *{product['name']}*\n"
                    f"💸 Сумма: *{price} тг*\n"
                    f"🔑 Ключей осталось: *{keys_left}*",
                    parse_mode="Markdown")
            except Exception:
                pass
        else:
        ...
        key_value = pop_product_key(dur_pool_key)
        set_balance(user_id, balance - price)
        save_order(user_id, item_key, f"{product['name']} ({label})", price)
        mark_key_buyer(dur_pool_key, user_id)
        ...
        if key_value:
            bot.edit_message_caption(
                caption=t['buy_success'].format(name=name_full, price=price, bal=balance-price, key=key_value, keys_left=keys_left),
                chat_id=chat_id, message_id=msg_id, reply_markup=kb, parse_mode="Markdown"
            )
            try:
                try:
...
def process_balance_edit_step1(message):
    ...
    username = f"@{user_row[2]}" if user_row[2] else "нет"
